...
def find_text_in_video(frame_iterator, find_text_in_frame_func):
    ...
    for sec, frame in frame_iterator:
        ...
        for blob in (b for b in old_blobs if len(b['change_frac']) == 5):
            if np.median(blob['change_frac'])>0.7: # seems stable between frames; take this as a real change
                other_blobs = [b for b in old_blobs if len(b['change_frac']) < 5 
                ...
                for b in other_blobs: 
                    ...
                    old_blobs.remove(b);
                yield largest_blob 
            old_blobs.remove(blob)
        
        for blob in (b for b in old_blobs if len(b['change_frac']) < 5): # compute the change frac for temp changes
            b = blob['blob']
            x, y = blob['left_corner']
            blob['change_frac'].append(img_proc_utils.changed_fraction(b, frame[x:x+b.shape[0],y:y+b.shape[1]], white_overweight=2))

        text_blobs = find_text_in_frame_func(frame, base_frame)
        for blob in text_blobs:
            #print blob
            blob.update({'frame': frame, 'sec':sec, 'change_frac':[]})
        old_blobs += text_blobs
